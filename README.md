...
To do work when a state is entered, define an `ENTER` handler. It receives the transition event, the new state, the context, and a bound `send` function. If it returns a function, that function runs as cleanup when the state is left, so timers and subscriptions don't leak:
...
const definition = {
  ...
  drag: {
    [ENTER]: (event, state, context, send) => {
      const id = setTimeout(() => send({ type: "mouseup" }), 3000);
